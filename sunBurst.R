...
chartVal <- ws5 %>% select(Itemset) %>%  group_by(Itemset) %>% mutate(chartVal = ((1/length(unique(Itemset)))* (1/n())))
inp <- ws5 %>% select(Cluster,Itemset,element) %>% apply(1,function(x) sapply(x,list))

D3partitionR(data=list(path=inp,value=ws4$ele_Weight),type="sunburst",tooltipOptions = list(showAbsolutePercent = F, showRelativePercent = F),trail = T)

for(i in 1:length(inp)) {
  str = ws4$ele_Weight[i] 
  for(j in 1:(str)) {
    inp[[i]][2+j] = inp[[i]][3] 
  }
}

D3partitionR(data=list(path=inp,value=ws5$ele_Weight),type="sunburst",tooltipOptions = list(showAbsolutePercent = F, showRelativePercent = F),trail = T)

noise = 0.05
n_thresh = floor(noise * n)
(ws6 <- ws5 %>% filter(ele_Weight>n_thresh)) %>% mutate(ele_Weight = ele_Weight - n_thresh)
inp <- ws6 %>% select(Cluster,Itemset,element) %>% apply(1,function(x) sapply(x,list))
for(i in 1:length(inp)) {
  str = ws6$ele_Weight[i] 
  for(j in 1:(str)) {
    inp[[i]][2+j] = inp[[i]][3] 
  }
}
#chartVal <- ws6 %>% mutate(chartVal = ((1/length(unique(Itemset))) * ())) %>% select(chartVal)
chartVal <- ws6 %>% select(Itemset) %>%  group_by(Itemset) %>% mutate(chartVal = ((1/length(unique(Itemset)))* (1/n())))

D3partitionR(data=list(path=inp,value=chartVal$chartVal),type="sunburst",tooltipOptions = list(showAbsolutePercent = F, showRelativePercent = F),trail = T)



#D3partitionR(data=list(path=inp,value=ws6$ele_Weight),type="sunburst",tooltipOptions = list(showAbsolutePercent = F, showRelativePercent = F),trail = T)

rad_chart <- ws5 %>% mutate(element_id = paste(Itemset,element, sep = "")) %>% 
  ggplot(aes(element_id,ele_Weight,fill = factor(Itemset))) + geom_bar(stat = "identity") + coord_polar()

#D3partitionR(data=list(path=inp,value=ws5$ele_Weight),type="indentedTree",specificOptions = list(bar = T))
